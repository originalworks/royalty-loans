...
describe('AgreementERC1155.safeTransferFrom', () => {
  ...
  it('can transfer to a holder', async () => {
    ...
    await agreement
      ...
      .safeTransferFrom(holder1.account, holder2.account, TOKEN_ID, 300, '0x00')

    expect(await agreement.balanceOf(holder1.account, TOKEN_ID)).to.equal(200)
    expect(await agreement.balanceOf(holder2.account, TOKEN_ID)).to.equal(800)
  })
  ...
  it('can transfer to a non-holder', async () => {
    ...
    await agreement
      ...
      .safeTransferFrom(holder.account, nonHolder, TOKEN_ID, 300, '0x00')

    expect(await agreement.balanceOf(holder.account, TOKEN_ID)).to.equal(700)
    expect(await agreement.balanceOf(nonHolder, TOKEN_ID)).to.equal(300)
  })
  ...
  it('can transfer shares to agreementERC1155', async () => {
    ...
    expect(await agreement.balanceOf(holder.account, TOKEN_ID)).to.equal(700)
    expect(await agreement.balanceOf(agreement2.address, TOKEN_ID)).to.equal(
      300,
    )
  })
  ...
  it('claims funds for sender and receiver before transfer', async () => {
    ...
    await agreement
      ...
      .safeTransferFrom(sender.account, receiver.account, TOKEN_ID, 500, '0x00')

    expect(await agreement.balanceOf(sender.account, TOKEN_ID)).to.equal(100)
    expect(await agreement.balanceOf(receiver.account, TOKEN_ID)).to.equal(900)
  ...
  it('does not affect third party holder in any matter', async () => {
    ...
    await agreement
      .connect(sender.wallet)
      .safeTransferFrom(sender.account, receiver.address, TOKEN_ID, 100, '0x00')
