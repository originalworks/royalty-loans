...
describe('AgreementERC20.removeAdmin', () => {
  ...
  it('sets the admin status to false', async () => {
    ...
    expect(await agreement.isAdmin(adminHolder.address)).to.be.true
    await agreement.connect(adminHolder).addAdmin(nonAdminHolder.address)
    expect(await agreement.isAdmin(nonAdminHolder.address)).to.be.true
    ...
    expect(await agreement.isAdmin(adminHolder.address)).to.be.false
  })
})
