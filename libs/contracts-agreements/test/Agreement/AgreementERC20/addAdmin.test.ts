...
describe('AgreementERC20.addAdmin', () => {
  ...
  it('fails if account is null account', async () => {
    ...
    await expect(agreement.addAdmin(ethers.constants.AddressZero)).to.be
      .reverted
  })
  ...
  it('sets the admin status', async () => {
    ...
    expect(await agreement.isAdmin(holder.account)).to.be.false

    const tx = await agreement.connect(admin.wallet).addAdmin(holder.account)

    expect(await agreement.isAdmin(holder.account)).to.be.true
