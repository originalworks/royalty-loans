...
import { deployLendingContract } from '../../scripts/actions/deployLendingContract'
...
import { addLender, buildHolders, getEvent } from './utils'
...
async function deployAgreementFactory(input: {
  ...
  const AgreementFactory = await ethers.getContractFactory('AgreementFactoryV3')
...
export async function deployInitialSetupLegacy(
  ...
  const lendingContract = await deployLendingContract(lendingToken.address)

  await addLender(lendingContract.address, lendingToken.contract, lender)
